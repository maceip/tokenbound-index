...
{#if provider}
	<div class="container">
		...
		<h2 class="heading-text">
			{#if title}{title}{:else}token <span>bound accounts</span>{/if}
		</h2>
		<ul class="image-gallery">
			{#each items as nft (nft.tokenAddress + '_' + nft.tokenID)}
				<LoadingNft {etherscanURL} {provider} tokenAddress={nft.tokenAddress} tokenID={nft.tokenID} account={nft.account}/>
			{/each}
		</ul>
		<PaginationNav
			{currentPage}
